...
public class ScanActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (connectionType == Connection.BLUETOOTH_CONNECTION) {
			...
			if (!bluetoothConnection.isBluetoothOpen()) {
				Toast.makeText(this, "Opening bluetooth", Toast.LENGTH_LONG).show();
			...
			public void onItemClick(AdapterView<?> av, View v, int position,
				...
				if (connectionType == Connection.WIFI_CONNECTION) {
					InetAddress server = (InetAddress) mArrayAdapter
							.getItem(position);
					try {
						Toast.makeText(ScanActivity.this, "Connection Host Adress :" + server.getHostAddress() , Toast.LENGTH_LONG).show();
	...
	private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
		public void onReceive(Context context, Intent intent) {
			...
			Toast.makeText(ScanActivity.this, "receiver bluetooth", Toast.LENGTH_LONG).show();
...
//				mArrayAdapter.add(f);
		...
		protected void onPostExecute(Void result) {
			...
			WifiConnection wifiConnection = (WifiConnection)WifiConnection.getConnection(Connection.WIFI_CONNECTION);	
			if(exp)
